...
contract OrbTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SORB }
    ...
    address public sORB;
    uint public sORBQueue; // Delays change to sORB address
    ...
    /**
        @notice allow approved address to deposit an asset for ORB
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ORB needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ORB ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ORB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IORBERC20( ORB ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sORB ).balanceOf( msg.sender ); // Can only borrow against sORB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ORB
        @param _amount uint
     */
    function repayDebtWithORB( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IORBERC20( ORB ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ORB, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ORB ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ORB ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ORB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ORB decimals
            value_ = _amount.mul( 10 ** IERC20( ORB ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SORB ) { // 9
            sORBQueue = block.number.add( blocksNeededForQueue );
        } else return false;
