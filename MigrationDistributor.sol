...
contract MigrateDistributor is Policy {
    ...
    address public immutable sOrbOld;
    
    address public newStaking;
    address public sOrbNew;
    ...
    */
    ...
    constructor( address _treasury, address _orb, uint _epochLength, uint _nextEpochBlock, address _oldStaking, address _sOrbOld, address _newStaking, address _sOrbNew ) {
        ...
        require( _sOrbOld != address(0) );
        sOrbOld = _sOrbOld;
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sOrbNew != address(0) );
        sOrbNew = _sOrbNew;
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochBlock <= block.number ) {
            ...
            uint oldReward= split(reward,ISORB(sOrbOld).circulatingSupply(),ISORB(sOrbNew).circulatingSupply());
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( ORB ).totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function setNewStaking(address _newStaking, address _sOrbNew) external onlyPolicy() {
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sOrbNew != address(0) );
        sOrbNew = _sOrbNew;
